    ...
    generate
        for (j = 0; j < REQS; j = j + 1) begin: foo
            if (j == 0) begin
                ...
                assign gnt_bus[2*WIDTH-1 -: WIDTH] =
                    tmp_gnts_rev[2*WIDTH-1 -: WIDTH] & ~tmp_gnts[WIDTH-1:0];
            end else begin
                assign tmp_reqs[(j+1)*WIDTH-1 -: WIDTH] =
                    tmp_reqs[(j-1)*WIDTH-1 -: WIDTH] &
...
module wand_sel #(parameter WIDTH = 64) (
    input  wire [WIDTH-1:0] req,
    ...
    generate
        ...
        for (i = 0; i < WIDTH-1; i = i + 1) begin : foo2
