...
Plan for addressing the three follow-up items left open by the `week3` merge (see `doc/week3-merge-report.md`). All work happens in a new git worktree.

## Context

After the week3 merge, three loose ends remained:

1. **`mult_no_lsq` nondeterminism** — the merged pipeline sometimes produces 44 correct writebacks in 45 seconds, other times produces 0 in 10+ minutes. Pre-existing bug inherited from milestone2 (which was previously unbuildable, so the bug was invisible until week3 unblocked integration testing).
2. **Missing `test/rob_test.sv`** — `verilog/rob.sv` has no unit test. `make rob.pass` fails at compile.
...
All three are now unblocked by week3 being in a known-good, buildable state. This plan addresses them in a new `week4` branch, with the cleanup (#3) sequenced last so the main worktree remains untouched while investigation and test-writing are in progress.

## Setup — new worktree

Create a new worktree rooted at the `week3` merge commit so the bugfix and new test land on top of the known-good merged state:
...
**Commit hygiene:** Each of the three tasks below produces one or more focused commits on `week4`. Use separate commits per task so bisect remains useful and cleanup can be reverted independently of the bugfix.
...
3. If `.wb` is consistently empty: the pipeline never commits a single instruction. That's a **different** failure mode than the 44-writeback run — record which one reproduces.
...
### Specific hypotheses to test (in rough likelihood order)
...
- `test/rs_test.sv` — use as a structural template for clock generation, reset, `@@@ Passed` / `@@@ Incorrect` printing, and any shared helpers.
...
6. **RAT stale-clear protection**
   Dispatch inst0 to reg 5 (tag 0), dispatch inst1 to reg 5 (tag 1) — now RAT points at tag 1. Complete inst0 via CDB, then commit inst0. Query reg 5. Verify `query1_pending == 1` and `query1_tag == 1` (RAT was NOT cleared by inst0's commit because it's no longer the latest writer).

7. **x0 guard**
   Dispatch inst writing to x0. Verify RAT never marks x0 as busy (query x0 → `pending == 0`). Also verify commit of an x0-destined instruction doesn't touch RAT state.
...
- `make rob.syn.pass` also prints `@@@ Passed` (synthesized ROB behaves identically).
...
The main worktree's `Makefile` has the stash-pop conflict markers **committed** into the `6f649b6` commit — they are not a local modification. This means the markers will remain unless the worktree is switched to a different branch (e.g., `week3`, which has them resolved).

### What to preserve

- `CLAUDE.md` — comprehensive project guide written during `/init`. Should live on `week3` (or `week4`) so future sessions find it.
...
1. **First, preserve the untracked content on `week3`.** From the week4 worktree:
   ...
   ```
   (This lands the docs on `week4`, which branches from `week3`. If the docs should live on `week3` itself instead, cherry-pick the commit back to `week3`: `git checkout week3 && git cherry-pick week4 && git checkout week4`.)
...
2. **Attach the main worktree to a proper branch.** From the main worktree directory:
   ...
   ```
   This detaches-to-attaches: the main worktree now tracks `week3`, which has the clean Makefile (stash-pop markers resolved) and will pick up the docs commit from step 1 if cherry-picked.

3. **Handle the `pdfs/` deletion.** After the checkout in step 2, the working-tree deletion should no longer appear (since `week3` and `week4` don't mark `pdfs/eecs4340project4.pdf` as modified relative to their tips). Verify with `git status`. If the deletion persists, decide:
...
2. **Task 2 first (rob_test.sv).** It's self-contained, gives fast feedback via `make rob.pass`, and doesn't touch the code paths involved in Task 1. Completing it first also provides a known-good test harness that might help surface Task 1's bug when run alongside the full pipeline.
3. **Task 1 second (`mult_no_lsq` debug).** The hard, open-ended one. Needs Verdi and iteration.
4. **Task 3 last (cleanup).** Best done once week4 has real commits to preserve. Touches the main worktree, so defer until there's nothing else to do there.
...
- Pushing any branch to `origin` — defer until the user decides the branches are ready for team review.
